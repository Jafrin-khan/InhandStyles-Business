...
const Cart = () => {
    ...
    const handleClick = () => {
        if (cart.length > 0) {
            const subject = 'Requirements';
            const body = `
                Products in Cart:
                ${cart.map(product => `${product.name} - Quantity: ${product.quantity} - Price: ${product.price * product.quantity} INR`).join('\n')}
                
                Total Items: ${cart.reduce((a, b) => a + b.quantity, 0)}
                Total Price: ${cart.reduce((a, b) => a + b.price * b.quantity, 0)} INR
            `;
            
            const recipients = ['jafrinkhan344@gmail.com', 'info@inhandstyles.co.in'].join(',');
    
            window.location.href = `mailto:${encodeURIComponent(recipients)}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
            // navigate('/shipping');
    
            Swal.fire(
                'Send the requirements via email'
            );
        } else {
            Swal.fire(
                'Error!',
                ...
                <div className="row">
                    ...
                    <div className="col-lg-3 mt-5 pb-2 d-none d-lg-block">
                        ...
                        <div onClick={() => handleClick()} style={{ border: '1px solid lightgrey' }} className="py-2">
                            <button id='btn_checkout' className='btn btn-secondary mx-auto d-block' disabled={disabled}>Send Requirements</button>
                        </div>
                    </div>
                    ...
                    <div style={{ boxShadow: '0 3px 10px 3px #0003' }} className="col-lg-3 mt-5 py-3 d-lg-none fixed-bottom bg-brand">
                        ...
                        <div onClick={() => handleClick()} style={{ border: '1px solid lightgrey' }} className="py-2">
                            <button id='btn_checkout_sm' className='btn btn-secondary mx-auto d-block' disabled={disabled}>Send Requirements</button>
                        </div>
                    </div>
